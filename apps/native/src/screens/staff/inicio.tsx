        ...
        <View style={{ gap: 8 }}>
          ...
          {home.upcoming.length === 0 ? (
            ...
            home.upcoming.map((ev) => {
              // Bug 14 — fila clicable a la ruta del área STAFF (entrenamiento→pasar
              // lista, partido→convocatoria). `other` → sin destino (no clicable).
              const target = staffEventTarget(ev);
              return (
                <ListCard
                  key={ev.id}
                  accent={accent}
