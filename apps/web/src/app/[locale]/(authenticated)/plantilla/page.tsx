...
/**
 * HUB "Plantilla": Jugadores, Importar, Cuerpo técnico y Equipos. Roles/hijos
 * desde `nav-config` (misma fuente que el sidebar); las rutas existentes no se
 * mueven (`/plantilla/importar` sigue donde estaba). El gating fino queda en
 * cada ruta destino.
 */
export default async function PlantillaPage({ params }: Props) {
  ...
  const children = getHubChildren('plantilla', role);
  if (children.length === 0) redirect(`/${locale}`);

  const t = await getTranslations('plantilla');
