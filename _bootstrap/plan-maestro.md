...
- **0.1** Inicializar el repositorio local + identidad git (15 min)
- **0.2** Estructura `docs/` + plantillas spec/ADR/retro (45 min)
- **0.3** Scaffold Turborepo monorepo: packages/core + apps/web (1–1.5 h)
- **0.4** apps/web Next.js + TS strict + Tailwind v4 + shadcn/ui + vínculo packages/core (45 min)
- **0.5** i18n con next-intl (es / en / va) (30 min)
- **0.6** Cliente Supabase en packages/core (sin schema todavía) (30 min)
- **0.7** Sentry SDK para Next.js (DSN en `.env.local`) (30 min)
- **0.8** PWA manifest + service worker básico (30 min)
- **0.9** GitHub Actions CI + PR template (30 min)
- **0.10** direnv `.envrc` + `.env.example` (15 min)
- **0.11** Plan Maestro como Markdown (este archivo) committeado a `docs/journey/plan-maestro.md` (15 min)
...
- **2.1** CRUD club, categorías, equipos — 1–2 h
