...
import { getSportingNamesFromClient } from './sporting-names';
 ...
 *
 * FETCH extraído de apps/web/.../equipos/[teamId]/team-stats-queries.ts (patrón 1):
 * el CÁLCULO ya vivía en core (aggregateTeamStats/splitMatchStatsByType/
 * aggregateTeamEventsByType/classifyMatchType); aquí se extrae el FETCH y se recibe
 * el cliente por parámetro. Todo LECTURA; la RLS es el gate. Comportamiento idéntico
 * al web. Usado por el web (wrapper) y por getPlayerBadgesFromClient (roster).
 *
 * El team es season-scoped (Rework C: `teams.season` es un único label por fila de
 * equipo), así que el roster = todas las membresías de `team_members` de ese team
 * (activas e históricas). Las stats son `match_player_stats` filtradas por `team_id`.
 *
 * Seguridad: se apoya en la RLS existente — `match_player_stats` (staff
 * user_can_record_match, propio-jugador, seguidor y COMPAÑERO de equipo
 * user_is_team_member_account F14E-6), `players`/`team_members`/`teams` (miembro del
 * club). No se añade política nueva.
 */
 ...
 * `stats` = métricas summables (Σ match_player_stats) por tipo. `matches` = partidos
 * reales (distinct event_id) por tipo. `events` (4b) = conteos de match_events por
 * tipo + total del rival. `goalsFor` (4b) = goles a favor del marcador por tipo;
 ...
 */
export async function getTeamSeasonStatsFromClient(
  supabase: DbClient,
  teamId: string,
  ...
  if (opts?.viewerIsSpectator) {
    ...
    const tmRows = tmOnly ?? [];
    const names = await getSportingNamesFromClient(
      supabase,
  ...
  // 3) match_player_stats del equipo (RLS: staff / propio-jugador / seguidor /
  // compañero de equipo). F9B-4a — se junta events!inner(type, tournament_id).
  ...
  // Agregado por jugador + totales: usa solo las columnas de match_player_stats.
  ...
  // 4) F9B-4b — eventos de equipo (match_events): corners, fueras de juego, faltas…
  // own + rival. Se filtra por el equipo del evento padre; RLS = user_can_record_match.
  ...
  // 5) F9B-4b — marcador (match_state): goles a favor por tipo (fila Goles del
  // equipo) + goles en contra (columna Rival). Misma regla de clasificación.
