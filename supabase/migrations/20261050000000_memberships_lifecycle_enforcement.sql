-- Baja de miembros (Paso 2 · ENFORCEMENT). Hace que `memberships.left_at IS NOT NULL`
-- signifique "ya no es miembro": la RLS le niega. El corte vive en LOS HELPERS, no en
-- las policies: añadir la guarda `left_at is null` a `user_role_in_club` propaga a las
-- 52 policies que lo usan sin editarlas. Aplica a jugadores (vía la membership del
-- TUTOR), staff y directores por igual. Reversible (left_at → NULL). La supresión RGPD
-- sigue siendo un flujo APARTE.
--
-- Cada función se RECREA VERBATIM desde su definición viva (pg_get_functiondef) + la
-- guarda marcada. Ninguna rama se reescribe. Ensayada con BEGIN...ROLLBACK y probes por
-- rol (director/coach/tutor/superadmin/reactivación/control) antes de aplicar.
--
-- Depende de 20261049 (columna memberships.left_at). El paso 3 (loader current-user.ts,
-- TS) y el paso 4 (acción de baja + guarda de admin) van aparte.

-- ── Helpers de rol/acceso ────────────────────────────────────────────────────────────

-- (1) user_role_in_club: la membership de un dado de baja deja de contar. Superadmin
--     intacto (atajo antes del subselect). → 52 policies heredan el corte.
  ...
  select case
    when public.is_superadmin() then 'admin_club'
      ...
      where club_id = p_club_id
        ...
        and left_at is null      -- baja de miembros: deja de ser miembro
      limit 1
    )
  end;
$function$;

-- (2) profile_is_staff_of_club: guarda en LAS DOS ramas (rol-membership y team_staff).
...
-- (3) user_coordinates_team: defensa en profundidad (aunque el team_staff siga abierto,
--     la baja del club corta el acceso de equipo).
...
-- (5) user_is_team_member_account: el TUTOR pierde el acceso al equipo del hijo cuando
--     SU membership está de baja. Invariante (verificado): todo tutor tiene membership
--     en el club del hijo → el join no deja fuera a nadie legítimo.
...
AS $function$
  select p_team_id is not null and exists (
    ...
    join public.memberships m on m.profile_id = pa.profile_id and m.club_id = t.club_id
...
-- ── Acción director/admin ────────────────────────────────────────────────────────────

-- (6) admin_update_staff_role: guarda SOLO en el SELECT del CALLER (un director de baja
--     no puede cambiar roles). El resto de ramas (owner, target, last-admin, high-role)
--     intactas.
...
begin
  ...
  -- Caller debe ser gestor del club: admin_club o director. Baja de miembros: ACTIVO.
...
-- ── Fan-out del chat de equipo ───────────────────────────────────────────────────────

-- (7) team_chat_member_profile_ids: un dado de baja no recibe el chat de equipo.
--     Guarda en las TRES ramas (staff, familia, admin/director).
...
AS $function$
  ...
  select m.profile_id
   ...
   where ts.team_id = p_team_id
     ...
     and m.left_at is null
  union
  -- jugador/familia del roster vigente (via player_accounts) con membership activa
  select pa.profile_id
   ...
   where tm.team_id = p_team_id
     and tm.left_at is null
  union
  -- admin/director del club del equipo con participación activa en el chat, ACTIVOS
...
-- ── Acciones solo-admin_club (defensa en profundidad) ────────────────────────────────
-- Hoy son seguras porque el admin_club no se da de baja (guarda de producto del paso 4),
-- pero la SEGURIDAD no debe depender de esa decisión: si alguien la cambia mañana, sin
-- esta guarda quedarían abiertas. Añadimos `and m.left_at is null` al gate admin_club.

-- (8) finalize_active_season
...
begin
  ...
  if not (public.is_superadmin() or exists (
    select 1 from public.memberships m
     where m.club_id = p_club_id and m.profile_id = v_uid and m.role = 'admin_club' and m.left_at is null
  )) then
    raise exception 'forbidden' using errcode = 'P0001';
  end if;
...
-- (11) unplace_player_from_upcoming
