...
| F1B | Rol director + owner único de club (extensión de F1; solo el owner gestiona roles altos, altos solo por invitación) | — | — | ☑ (2026-07-06) |
| F5B | Comunicación del club — chat de equipo grupal + mejoras de mensajería (extensión de F5) | — | — | ☑ (2026-07-07) |
| F7B | Pantalla "Directos" (consulta del directo, todos los roles) + alertas de goles a seguidores (extensión de F7/F13B) | — | — | ☑ (2026-07-08) |
...
> **Cambio 2026-07-06 (cierre F1B — Rol director + owner de club)**: ☑ **F1B entregada** con etiqueta de letra (**extensión de F1** — Identidad, Auth y modelo de roles base; no renumera F14–F16, mismo patrón que F13.10/F13B/F9B). Cierra el punto de backlog **"Owner de club"** (§ Gestión multi-club, diferido desde F9). **Modelo resultante**: (1) **`admin_club` = owner único** por club vía nueva columna `clubs.owner_profile_id` (backfill = primer admin por `created_at`); el owner es un `admin_club` **inmutable** (no degradable ni eliminable, ni por sí mismo) y es la protección estructural del "último admin". (2) **`director`** = 6º valor de `memberships.role` con **paridad TOTAL de datos con `admin_club`** (helper central `user_is_admin_or_director`, barrido comprehensivo de ~40 objetos WRITE) **salvo** gestionar directores/admins. (3) **Los roles altos (director/admin_club) solo se alcanzan por INVITACIÓN**, nunca por cambio de rol — para NADIE, ni el owner (`admin_update_staff_role` lo rechaza con `high_role_invite_only`); **invitarlos/gestionarlos es exclusivo del owner** (`user_is_club_owner`; degradar un alto = potestad del owner). (4) **UI**: `isOwner` expuesto en `CurrentUserClub`, constantes de rol **centralizadas en `@misterfc/core/auth/roles`** (ADMIN/MANAGER/STAFF/COACH/ALL_CLUB — `director` en todas salvo COACH), selector de invitación que ofrece `director` solo al owner, y barrido de ~55 ficheros que desbloquea al director en gates de UI/navegación. Serie **F1B-0…F1B-3d** = PRs **#274** (F1B-0 enum `director` + `owner_profile_id` + `user_is_club_owner`), **#275** (F1B-1 paridad de datos, `user_is_admin_or_director`), **#276** (F1B-2 solo owner gestiona altos), **#277** (F1B-2b altos solo por invitación), **#278** (F1B-3a+b UI acceso + centralización), **#279** (F1B-3c+d crear directores desde la UI), **#280** (fix regresión de aceptación de invitación) — todas mergeadas 2026-07-06. **Sin migración de datos** (solo DDL de policies/columna). Red de pruebas consolidada en `supabase/tests/rls_f1b_director_role.sql` (**F1B-4**, 43 comprobaciones: aislamiento + equivalencia + jerarquía + anti-regresión del owner + aceptación por invitee). Detalle en §6 (tras F9B) y en [progress.md](progress.md). Sin delta de horas (entregado, no estimado a futuro).

> **Cambio 2026-07-07 (cierre F5B — Comunicación del club)**: ☑ **F5B entregada** con etiqueta de letra (**extensión de F5** — Mensajería interna; no renumera F14–F16, mismo patrón que F13.10/F13B/F9B/F1B). Añade el **chat de equipo grupal** (que F5 no cubría — F5 era 1:1 + anuncios + push) y varias mejoras de mensajería. **Modelo nuevo**: tablas `team_conversations` (una por equipo) + `team_messages` **separadas** del 1:1; **miembros DERIVADOS** en la RLS (staff ∪ jugador/familia del roster vigente ∪ admin/director del club), sin tabla de participantes, auto-sincronizados con altas/bajas; aislamiento estricto por equipo/club. **Supervisión de dirección**: el director entra a todos los chats en **modo observer** (VE todo pero **no escribe ni recibe notificaciones**) y puede **activar** la participación por chat (`team_chat_participation`); el gate es a nivel **RLS** (helper `user_can_post_team_chat`: staff/jugadores siempre, director solo si `active`), y el mismo criterio rige el **fan-out de notificaciones** y los **no-leídos**. **No-leídos por grupo** vía `team_conversation_reads` (marca watermark por usuario/chat; badge en `/mensajes` al nivel del 1:1). **Sin realtime**: auto-refresco por **polling ~5s** (grupo y 1:1), en pausa si la pestaña se oculta. Serie **F5B-0…F5B-5** = PRs **#282** (F5B-0 pestaña "Equipos" top-level), **#283** (F5B-1 iniciar chat 1:1 con buscador de jugadores), **#284** (F5B-2 modelo de chat de equipo + RLS derivada), **#285** (F5B-3 UI del chat de equipo + iniciar + fan-out), **#287** (fix scope del selector por rol + sin duplicados por temporada), **#286** (F5B-3b polling ~5s), **#288** (F5B-4 supervisión/observer + `team_chat_participation`), **#289** (F5B-5 no-leídos por grupo + `team_conversation_reads`) — todas mergeadas 2026-07-06/07. **Migraciones aditivas** (3: F5B-2, F5B-4, F5B-5; sin migración de datos). pgTAP de dry-run contra el remoto para la RLS derivada, el observer y los no-leídos. Detalle en §6 (tras F1B) y en [progress.md](progress.md). Sin delta de horas (entregado, no estimado a futuro).

> **Cambio 2026-07-08 (cierre F7B — Pantalla "Directos" + alertas de goles)**: ☑ **F7B entregada** con etiqueta de letra (**extensión de F7** — Toma de datos en directo / F13B — Gestión de partidos; no renumera F14–F16, mismo patrón que F13.10/F13B/F9B/F1B/F5B). Es la **cara de consulta** del directo para **todo el club** (F7 solo lo ve el staff que graba) más **alertas push de goles**. **Decisiones de producto fijadas**: (1) **"Directos" es una sección NUEVA E INDEPENDIENTE** (`/directos`, entrada de menú propia para **todos los roles**) — **no toca** "Gestión de partidos" (F13B) ni el **directo del staff** (F7); solo **lectura**. (2) **Todos ven TODOS los partidos de SU club** (marcador, fase/reloj, eventos y **alineación en el campo**), sin filtrar por el equipo del hijo; **aislamiento estricto solo ENTRE clubs**. (3) Lista de los partidos de la **semana natural (lun–dom)** en **orden cronológico por hora de inicio** (el estado —en directo/sin empezar/finalizado— se ve por el badge), con **fecha/hora por partido**; refresco por **polling ~5s** de los vivos. (4) **Fase + minuto de fútbol reconstruibles desde la BD** (reloj de servidor ya persistido en F7.1) vía **helper puro `matchPhase`** en core; congelado en descanso/fin. (5) **Push de gol a los SEGUIDORES del equipo** (`team_follows`) para **CUALQUIER gol del partido** (nuestro **o** rival); mensaje **"Gol"** + **"{Local N − M Visitante}"** con el marcador ya actualizado (`computeScore`), deep link a `/directos/{eventId}`; excluye al que registra el gol, **dedupe idempotente por gol**, respeta `notification_preferences`/`push_subscriptions`; **solo goles EN VIVO** (la edición retroactiva `addMatchEvent` no emite). (6) **Seguir/dejar de seguir es el interruptor** del aviso (pantalla `/directos/seguir`, todos los equipos del club); **no** hay toggle `'goal'` en la matriz de preferencias (anotado como posible mejora en [known-issues.md](known-issues.md)). Nuevo `notification_type 'goal'`. Serie **F7B-0…F7B-P1** = PRs **#291** (F7B-0 helper puro `matchPhase` fase+minuto), **#292** (F7B-2 abrir RLS de LECTURA del directo a todo el club: `match_state`/`match_events`/`match_periods`/`match_starters`/`lineups`/`lineup_positions`), **#293** (F7B-3 pantalla "Directos": lista + detalle con campo/stats/eventos/minuto), **#294** (mejoras de lista: fecha/hora por partido + orden cronológico), **#295** (F7B-P1 seguir equipos `team_follows` + push de goles) — todas mergeadas 2026-07-07/08. **Migraciones aditivas** (2: RLS de lectura F7B-2; `team_follows` + enum `goal` F7B-P1; sin migración de datos). Tests: unit de core (`formatGoalPush`/`resolveGoalRecipients`) + pgTAP de dry-run `rls_team_follows.sql` (aislamiento + resolución de seguidores). Detalle en §6 (tras F5B) y en [progress.md](progress.md). Sin delta de horas (entregado, no estimado a futuro).
...
**Dependencias**: Fase 9 cerrada. *(Nota 2026-06-09 — **superada: F9 cerrada el 2026-06-12** con el segundo tramo 9.B; se conserva como contexto histórico de la dependencia de F10 en su arranque: el **núcleo de F9 estaba hecho** — stats agregadas, ratios, evolución y vista jugador/familia disponibles y reutilizables — pero entonces **F9 no estaba cerrada del todo**: faltaban el multi-temporada y los PDF del segundo tramo. F10 se apoyó en los agregados/helpers del núcleo.)*
...
**F1B — Rol director + owner único de club** ☑ **[cerrada 2026-07-06]** *(extensión de F1; etiqueta de letra, no renumera F14–F16)*:

> Cierra el punto de backlog **"Owner de club"** (§ Gestión multi-club). Añade un **rol `director`** con **paridad total de datos con `admin_club`** salvo gestionar directores/admins, y consolida el **owner único** por club (`clubs.owner_profile_id`) como el único que administra roles altos. **Solo DDL** (policies + columna + helpers); sin migración de datos. Detalle por PR en [progress.md](progress.md) §"F1B — Rol director + owner de club".

- **Fundación** ☑ — **#274** (F1B-0: `director` = 6º valor de `memberships.role`; columna `clubs.owner_profile_id` con backfill al primer admin; helper `user_is_club_owner`; **ninguna policy de permiso cambia** aún).
- **Paridad de datos** ☑ — **#275** (F1B-1: helper `user_is_admin_or_director`; barrido comprehensivo de ~40 objetos WRITE para que `director` escriba donde escribe `admin_club`; excluye el Grupo B — gestión de roles).
- **Jerarquía (Grupo B)** ☑ — **#276** (F1B-2: invitar/gestionar roles altos = **solo el owner**; owner inmutable; helpers `membership_role_is_high`, `profile_is_club_owner`) + **#277** (F1B-2b: subir a director/admin por cambio de rol **rechazado para todos** incluido el owner → `high_role_invite_only`; degradar un alto sigue siendo del owner).
- **UI** ☑ — **#278** (F1B-3a+b: `isOwner` en `CurrentUserClub`; constantes de rol centralizadas en `@misterfc/core/auth/roles`; barrido de ~55 ficheros que abre al director gates de UI/navegación) + **#279** (F1B-3c+d: selector de invitación ofrece `director` solo al owner; diálogo de cambio de rol visible a admin/director y solo con roles bajos como destino).
- **Fix de regresión** ☑ — **#280** (F1B-2 había sustituido en la policy de aceptación de invitación el helper `current_user_email()` por un subquery inline a `auth.users`, que `authenticated` no puede leer → `permission denied` rompía el alta de **cualquier** invitado; vuelto al helper `SECURITY DEFINER`).
- **Red de pruebas + cierre** ☑ — **F1B-4** (suite pgTAP consolidada `supabase/tests/rls_f1b_director_role.sql`, 43 comprobaciones; este cierre documental).

**Invariantes del modelo** (para features futuras que toquen roles):

- **`admin_club` = owner único** por club (`clubs.owner_profile_id`). El owner es un `admin_club` **inmutable** — no degradable ni eliminable (ni por sí mismo) → es la protección estructural del "último admin".
- **`director` = paridad TOTAL de datos con `admin_club`** salvo gestionar directores/admins. Gate de datos = helper `user_is_admin_or_director`; gate de roles altos = `user_is_club_owner`.
- **Roles altos (director/admin_club) SOLO por invitación**, nunca por cambio de rol — para NADIE, ni el owner. Invitarlos/gestionarlos es **exclusivo del owner**.
- **Constantes de rol centralizadas** en `@misterfc/core/auth/roles` (ADMIN/MANAGER/STAFF/COACH/ALL_CLUB); `director` entra en todas **salvo** COACH. Cualquier gate de UI nuevo debe usar estas constantes, no listas locales.

---

**F5B — Comunicación del club (chat de equipo)** ☑ **[cerrada 2026-07-07]** *(extensión de F5; etiqueta de letra, no renumera F14–F16)*:

> Añade el **chat de equipo grupal** que F5 no cubría (F5 = mensajería 1:1 + anuncios + push) y varias mejoras de mensajería. **Modelo nuevo separado del 1:1**: `team_conversations` (una por equipo) + `team_messages`, con **miembros DERIVADOS** en la RLS (staff ∪ jugador/familia del roster vigente ∪ admin/director del club) — sin tabla de participantes, auto-sincronizados con altas/bajas, aislamiento estricto por equipo/club. **Sin realtime** (polling ~5s). Detalle por PR en [progress.md](progress.md) §"F5B — Comunicación del club (chat de equipo)".

- **Navegación** ☑ — **#282** (F5B-0: pestaña **"Equipos"** top-level en el sidebar para dirección).
- **Iniciar 1:1** ☑ — **#283** (F5B-1: iniciar chat directo desde Comunicaciones con **buscador de jugadores** del club).
- **Modelo de chat de equipo** ☑ — **#284** (F5B-2: tablas `team_conversations` + `team_messages`, helpers de pertenencia derivada `user_is_team_chat_member[_by_conversation]`, RLS bidireccional, resolutor de fan-out `team_chat_member_profile_ids`).
- **UI del chat de equipo** ☑ — **#285** (F5B-3: hilo de grupo con optimistic UI + nombre del emisor, iniciar desde la ficha del equipo y desde Comunicaciones, fan-out de notificaciones reusando `new_message`) + **#287** (fix: el selector de equipos filtra por **rol** — entrenador solo sus equipos vía `team_staff`, director todos — y por **temporada activa**, sin duplicados por temporada tras Rework A).
- **Auto-refresco** ☑ — **#286** (F5B-3b: **polling ~5s** en grupo y 1:1 con hook compartido `useVisibleInterval`; pausa al ocultar la pestaña; merge de optimistas; scroll-stick solo si estás al fondo; **sin Supabase Realtime**).
- **Supervisión / observer** ☑ — **#288** (F5B-4: `team_chat_participation`; el director entra en **modo observer** por defecto — ve todo pero **no escribe ni recibe** — y activa la participación por chat; el INSERT y el fan-out pasan por `user_can_post_team_chat`: staff/jugadores siempre, director solo si `active`; el SELECT no cambia).
- **No-leídos por grupo** ☑ — **#289** (F5B-5: `team_conversation_reads` marca watermark por usuario/chat; RPC `team_chat_unread_counts`; badge en `/mensajes` al nivel del 1:1; el director solo acumula no-leídos en los chats donde **participa** — mismo criterio que las notificaciones).

**Invariantes del modelo** (para features futuras que toquen el chat de equipo):

- **Pertenencia DERIVADA, no materializada**: quién es miembro de un chat de equipo se computa en la RLS (`user_is_team_chat_member`), no hay tabla de participantes. `team_chat_participation` y `team_conversation_reads` son estado **por usuario** (observer/lectura), no la lista de miembros.
- **Un único criterio de "participa"**: `user_can_post_team_chat(team)` = staff ∪ jugador/familia ∪ (admin/director **con** `active`). Gobierna a la vez **escritura** (RLS INSERT), **notificaciones** (fan-out) y **no-leídos**. El **SELECT es más amplio** (el director ve todo aunque solo observe) y NO debe alinearse con "participa".
- **Sin realtime**: la frescura del hilo es por **polling ~5s**; cualquier mejora de tiempo real futura sustituye el hook, no el modelo.
...
- **Owner de club** ☑ **[entregado en F1B, 2026-07-06]** (sucesor natural de la guarda del último admin, Bug 2·2b — PR #116): un **admin protegido no degradable** (el "dueño" del club, distinto de un `admin_club` cualquiera). Entregado como `clubs.owner_profile_id` + owner inmutable + gestión de roles altos exclusiva del owner (ver **F1B** en §6 y en la tabla). **Pendiente aún**: la **transferencia de propiedad** explícita (cambiar el owner de un club) no se implementó — entra con el multi-club / god user.

> **Owner de club: entregado en F1B** (2026-07-06). Queda pendiente solo el flujo de **traspaso de propiedad**. El **god user / superuser de plataforma** sigue diferido (sin estimación; entra con su propia spec cuando el multi-club deje de ser hipotético).
...
| **BC-8a** | Notas de revisión Apple + cuenta de prueba + propuesta de texto legal | — | ✅ #572 [hecho 2026-09-11] |
| **BC-8b** | SQL: la consola ve los clubes SIN administrador (métricas de miembros activos) | ✅ un fichero | ✅ #573 [hecho 2026-09-11] |
| **BC-8c** | Consola de plataforma: badge «Sin administrador» + aviso de cabecera | — | ☐ |
